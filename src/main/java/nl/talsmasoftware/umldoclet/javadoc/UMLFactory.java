...
import nl.talsmasoftware.umldoclet.annotations.UML;
...
public class UMLFactory {
    ...
    public UMLRoot createClassDiagram(TypeElement classElement) {
        ...
        if (!TypeKind.NONE.equals(classElement.getSuperclass().getKind())) {
            ...
            if (!isExcluded(superclassName, superclass)) {
        ...
        for (TypeMirror interfaceType : classElement.getInterfaces()) {
            ...
            if (!isExcluded(ifName, implementedInterface)) {
        ...
        if (enclosingKind.isClass() || enclosingKind.isInterface()) {
            ...
            if (!isExcluded(enclosingTypeName, enclosingElement)) {
    ...
    private boolean isExcluded(TypeName typeName, Element element) {
        requireNonNull(typeName, "Type name is <null>.");
        if (config.excludedTypeReferences().contains(typeName.qualified)) {
            return true;
        }
    ...
    private Collection<Reference> findPackageReferences(
        ...
        if (!TypeKind.NONE.equals(typeElement.getSuperclass().getKind())) {
            ...
            Element superclassElem = env.getTypeUtils().asElement(typeElement.getSuperclass());
            if (!isExcluded(superclass, superclassElem)) {
        ...
        typeElement.getInterfaces().forEach(interfaceType -> {
            ...
            Element interfaceElem = env.getTypeUtils().asElement(interfaceType);
            if (!isExcluded(interfaceName, interfaceElem)) {
    ...
    Namespace createPackage(UMLRoot diagram,
        ...
        packageElement.getEnclosedElements().stream()
                .filter(TypeElement.class::isInstance).map(TypeElement.class::cast)
                .flatMap(UMLFactory::innerTypes)
                .filter(typeElement -> !isExcluded(TypeNameVisitor.INSTANCE.visit(typeElement.asType()), typeElement))
                .map(typeElement -> {
